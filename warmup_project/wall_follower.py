...
class WallFollowerNode(Node):
    ...
    def run_loop(self):
        """
        Main loop for wall following behavior. Adjusts velocity based on scan data \
        and wall proximity to either follow the closest wall or turn when approaching an obstacle.
        This is a CoPilot-generated docstring (that we checked for accuracy).
        """
        msg = Twist()
        ...
        if self.flmean != self.prev_f:
            if (
                ...
                if abs(self.rfmean - self.rbmean) < 0.01 or abs(self.rfmean - self.rbmean) > 0.3:
                    ...
                    msg.angular.z = float(
                        self.rfmean - self.rbmean) * self.ang_vel
                self.vel_pub.publish(msg)
    ...
    def process_scan(self, data):
        ...
        self.flmean = 100 if len(self.distances[front_left]) == 0 else sum(
            self.distances[front_left]) / len(self.distances[front_left])
        self.frmean = 100 if len(self.distances[front_right]) == 0 else sum(
            self.distances[front_right]) / len(self.distances[front_right])
        self.lmean = 100 if len(self.distances[left]) == 0 else sum(
            self.distances[left]) / len(self.distances[left])
        self.lfmean = 100 if len(self.distances[left_front]) == 0 else sum(
            self.distances[left_front]) / len(self.distances[left_front])
        self.lbmean = 100 if len(self.distances[left_back]) == 0 else sum(
            self.distances[left_back]) / len(self.distances[left_back])
        self.rmean = 100 if len(self.distances[right]) == 0 else sum(
            self.distances[right]) / len(self.distances[right])
        self.rfmean = 100 if len(self.distances[right_front]) == 0 else sum(
            self.distances[right_front]) / len(self.distances[right_front])
        self.rbmean = 100 if len(self.distances[right_back]) == 0 else sum(
            self.distances[right_back]) / len(self.distances[right_back])
